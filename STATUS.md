...
- Database migrations through `20260825100000_restore_session_notes` are applied locally.
...
## Verification
- `npm test` passes: 3 tests, including focused last-time ownership, temporal, set-kind, one-rep, tie, and session-notes coverage.
- `npm run build` passes for client and server.
- `npm run lint` passes.
- `npm run typecheck` passes.
- `npm run migrate:deploy -w server` applies the minimal session-notes migration; Prisma reports ten migrations and no pending work.
