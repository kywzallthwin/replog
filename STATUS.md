...
## Current Phase
- `/program` routine editor backend + client are built and API-verified; forgot/reset password recovery is built and API-verified except for real Resend delivery; a static responsive/accessibility polish pass is complete across the existing shell, Program, Progress, Profile, History, Dashboard, and Workout UI. Rendered browser spot-checks at the target viewport sizes are still pending (see Next Actions) / `/guide` is explicitly skipped (see Backlog) / remaining feature backlog items are unscheduled
...
- Client now has a persistent navigation shell: `client/src/components/nav/navItems.ts`, `BottomTabBar.tsx` (fixed through tablet widths, `lg:hidden`), and `TopNav.tsx` (inline at desktop widths, `hidden lg:flex`)
...
- Responsive shell polish now reserves tab-bar plus safe-area space, uses dynamic viewport height, and enables `viewport-fit=cover`; existing high-frequency controls use 44px minimum targets and global keyboard focus is visible
- Program and Workout dialogs now have dialog semantics, labelled headings, Escape dismissal, selected-state semantics, and short-viewport constrained scrolling; Program also handles long names, enforces the 60-character day-name limit, and displays reorder errors
- Workout numeric fields request decimal/numeric keyboards and mirror server limits; completed workouts are labelled correctly, Progress workout links preserve `?from=progress`, and the Progress table scrolls safely on narrow screens
- Profile now displays real dashboard stats and links to Edit Program; dashboard recent-session data includes `endedAt` so zero-duration finished sessions are not shown as active
- Removed the pre-existing `EditProfilePage` synchronous state-setting effect so the current React hooks lint rules pass
- Public `/forgot-password` and `/reset-password` client routes now match the auth mockup, and Login links to the recovery flow
- Backend password recovery now uses one-hour, single-use random tokens stored only as SHA-256 hashes; generic forgot-password responses avoid disclosing whether an account exists
- Password reset increments `User.authVersion`; JWTs include that version and protected-route middleware checks it, invalidating every session issued before a successful reset
- Resend email delivery and per-IP forgot/reset rate limits are wired; local delivery requires `RESEND_API_KEY`, while `EMAIL_FROM` defaults to `RepLog <onboarding@resend.dev>` for Resend account-owner testing
...
- Completed a static visual/responsive/accessibility polish pass across the existing UI: hardened navigation breakpoints and safe-area spacing, 44px tap targets, focus treatment, Program/Workout dialog sizing and semantics, numeric workout inputs, Progress source links/table overflow, real Profile stats/Edit Program link, completed-workout labels, and dashboard finished-session detection. Also fixed the pre-existing EditProfile React hooks lint violation. Verified with `npm run build -w client`, `npm run lint -w client`, `npm run typecheck -w server`, and `git diff --check`
- Added complete forgot/reset password recovery: Prisma migration, hashed expiring reset tokens, Resend delivery adapter, route-specific rate limits, generic request responses, single-use reset handling, all-device JWT revocation through `authVersion`, mockup-aligned public client pages, and the Login recovery link. Verified Prisma validate/generate/migration, client build/lint, server typecheck, `git diff --check`, and a 15-assertion API smoke test covering validation, successful reset, token consumption, expiry/reuse rejection, old-password rejection, new-password login, and old-session revocation
...
5. [ ] Manual browser spot-check at 375px, about 768px, and 1080px+ of History/Progress empty and populated states, month grouping, Progress table/source links, and Workout exercise-picker/confirmation dialogs. Static overflow, target-size, keyboard-input, and dialog-shell issues were addressed in the 2026-08-15 polish pass, but rendered behavior still needs confirmation.
6. [ ] Manual browser spot-check of the persistent navigation shell across `/dashboard`, `/history`, `/progress`, `/program`, and `/profile`: confirm active highlighting/navigation, bottom bar through tablet widths, TopNav at desktop width, safe-area clearance, no obscured final content, and keyboard focus visibility.
7. [ ] Manual browser spot-check of `/program`: add/rename/recolor/delete a day, add/remove/reorder exercises, confirm up/down disable at list ends, exercise names remain usable at 375px, dialogs remain reachable on a short viewport, Dashboard updates after edits, and the Program nav item highlights correctly. Backend logic and static client checks are already verified.
8. [ ] Add `RESEND_API_KEY` to `server/.env`, use the Resend account-owner email with the default onboarding sender, and manually verify receipt plus the full `/forgot-password` -> email -> `/reset-password` browser flow. Replace `EMAIL_FROM` after a production sender domain is verified.
...
- `Replog-mockup/Replog-mockup.html` was rewritten as a forward-looking design spec (its own note says so) and now specs several features not yet built. Not scheduled yet — capture only, to be turned into real Next Actions in a future session:
  ...
  - `/forgot-password` and `/reset-password` routes: DONE; real Resend delivery setup remains in Next Actions.
  ...
  - Profile additions: kg/lb units toggle. The Edit Program settings row is DONE.
  - Progress page: SVG estimated-1RM trend chart above the session-history table.
  - Mobile-first rules called out in the mockup note: safe-area-inset-bottom padding, 44px minimum tap targets, and numeric keyboards are DONE for the existing targeted UI as of the 2026-08-15 polish pass. The rest-timer visibility rule remains deferred because the rest timer feature itself is not built.
...
## Blockers
- Real password-reset email delivery cannot be verified until a valid `RESEND_API_KEY` is added to ignored `server/.env`; API reset behavior has been verified independently.
...
- 2026-08-15: Completed a static visual/responsive/accessibility polish pass for the existing app. Moved the bottom-nav/TopNav switch from `sm` to `lg`, added safe-area content reservation and `viewport-fit=cover`, dynamic viewport heights, 44px targets, stronger nav contrast, and global focus-visible outlines. Hardened Program and Workout dialogs for short viewports with semantics, labelled headings, selected-state semantics, Escape handling, and scrollable bodies; added Program long-name containment/day-name max length/reorder errors; added workout numeric input modes and server-matching limits; corrected completed workout labels; linked Progress PB/session rows back through `?from=progress`; made the Progress table horizontally safe; populated Profile stats from the dashboard and added Edit Program; and exposed dashboard `endedAt` so zero-duration finished sessions are not shown as active. The first lint run exposed a pre-existing `EditProfilePage` set-state-in-effect violation, which was removed with nullable controlled overrides. Final `npm run build -w client`, `npm run lint -w client`, `npm run typecheck -w server`, and `git diff --check` all passed. Rendered viewport checks remain open because this session had no reliable browser automation.
- 2026-08-15: Built forgot/reset password recovery from the forward-looking mockup. Added the `add_password_reset` Prisma migration (`authVersion`, reset-token hash/expiry), Resend plus per-IP rate limiting, `POST /auth/forgot-password` and `POST /auth/reset-password`, one-hour single-use SHA-256-hashed tokens, generic account-enumeration-safe request responses, and JWT auth-version checks that revoke all older sessions after reset. Added public `/forgot-password` and `/reset-password` pages, auth API helpers, configurable auth-shell taglines, and the Login link. Prisma validate/generate/migration, server typecheck, client build/lint, `git diff --check`, and a disposable-account 15-assertion API smoke test all passed. Actual email delivery remains pending a valid local `RESEND_API_KEY`; the onboarding sender restriction and setup are documented in `.env.example` and Next Actions.
